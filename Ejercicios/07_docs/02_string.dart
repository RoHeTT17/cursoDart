main(List<String> args) {
  ...
  print('Operador []: ${ nombreCompleto[10] }'); //Obtener el caracter de tal posición
  print('Operador *:  ${ nombreCompleto * 2 }');  //Imprime N veces la cadena
  print('Operador *:  ${ '*' * 10 }');

  print('ReplaceAll: ${ nombreCompleto.replaceAll(RegExp(r'e'), 'a') }'); // todas las e por a
  print('SubString:  ${ nombreCompleto.substring(0, 5) }...'); //hacel substring y adiciona los ...
  print('indexOf F:  ${ nombreCompleto.indexOf('F') }'); // Fernando

  print('Split: ${ nombreCompleto.split(' ') }'); // Fernando Herrera
  print('Split: ----${ nombreCompleto.split(' ')[1] }----'); //[1] la posición que queramos que salga 
